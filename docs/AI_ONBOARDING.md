...
- **阶段**：M0 全部完成——CLI 错误处理（loop 状态回滚 + 前端展示）、provider `APIError` 收尾、pytest 最小测试（5 用例全绿，`tests/test_loop.py`），已在 `feat/m0-kernel-skeleton` 提交。
- **下一步**：进入 M1 钩子系统，把 `loop.py` 的 `_emit` 空广播接入 `HookManager`；范围与验收见 `docs/ROADMAP.md`。
